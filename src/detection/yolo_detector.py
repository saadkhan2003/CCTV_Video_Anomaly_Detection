...
class YOLOAnomalyDetector:
    ...
    WEAPON_CLASSES = [43, 34, 76]   # knife, baseball bat, scissors (often detected as sharp object)
        ...
        if device == "cpu":
            try:
                ...
                if not os.path.exists(openvino_path):
                    print("🚀 Exporting model to OpenVINO for 2-3x CPU speedup... (This takes ~1 min once)")
    ...
    def detect_objects(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        results = self.model.track(frame, conf=self.confidence_threshold, persist=True, verbose=False, tracker="bytetrack.yaml")
    ...
    def check_anomalies(
        ...
        if person_count >= self.crowd_threshold:
            ...
            anomalies["risk_level"] = "high" if person_count > self.crowd_threshold * 2 else "medium"
        
        # Rule 2: FIGHT/CONFLICT DETECTION - People in close proximity
        if person_count >= 2:
            close_pairs = self._check_proximity(detections["persons"], proximity_threshold=150)
            if close_pairs > 0:
                anomalies["is_anomaly"] = True
        ...
        if len(detections["weapons"]) > 0:
            ...
            anomalies["anomaly_details"].insert(0, f"⚠ WEAPON DETECTED: {', '.join(weapon_names)}") # Priority msg
    ...
    def _check_proximity(self, persons: List[Dict], proximity_threshold: int = 150) -> int:
        """
    ...
    def draw_annotations(
        ...
        for det in detections["vehicles"]:
            x1, y1, x2, y2 = det["bbox"]
            cv2.rectangle(frame_copy, (x1, y1), (x2, y2), (255, 165, 0), 2)  # Orange for vehicles
        ...
        # cv2.rectangle(frame_copy, (pad, 10), (pad + 200, 40), status_color, -1)
        
        # Text
        cv2.putText(frame_copy, status_text, (pad, 35),
                   cv2.FONT_HERSHEY_DUPLEX, 0.8, status_color if not anomalies["is_anomaly"] else (0, 0, 255), 1)
        
        cv2.putText(frame_copy, detail_text, (pad + 250, 35),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220, 220, 220), 1)

        # Stats on right side
        stats_text = f"P: {len(detections['persons'])} | V: {len(detections['vehicles'])}"
        if self.frame_count % 30 < 15: # Blinking effect for recording dot? No, just clean text
            pass
            
        (tw, th), _ = cv2.getTextSize(stats_text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1)
        cv2.putText(frame_copy, stats_text, (w - tw - pad, 35),
        ...
        if anomalies["is_anomaly"] and anomalies["anomaly_details"]:
            ...
            cv2.rectangle(overlay_bottom, (0, h - 40), (w, h), (0, 0, 150), -1) # Red tint
            cv2.addWeighted(overlay_bottom, 0.6, frame_copy, 0.4, 0, frame_copy)
            
            detail_msg = anomalies["anomaly_details"][0]
            cv2.putText(frame_copy, detail_msg, (pad, h - 12),
    ...
    def process_video(
        ...
        # Output video - Browser compatible MP4
        ...
        while True:
            ...
            if frame_num % 3 == 0:
                annotated_frame, results = self.process_frame(frame, draw_detections=True, fps=fps)
        ...
        stats["anomaly_rate"] = stats["anomaly_frames"] / stats["total_frames"] if stats["total_frames"] > 0 else 0
...
def get_yolo_detector(
    ...
    if _detector_instance is None:
        _detector_instance = YOLOAnomalyDetector(
            model_size="s",  # Small for better speed/accuracy trade-off
