...
def find_anomaly_regions(
    error_heatmap: np.ndarray, threshold_percentile: float = 90, min_area: int = 50
) -> List[Dict]:
    """
...
def draw_bounding_boxes(
    ...
    for i, region in enumerate(regions[:3]):
        ...
        # Bottom-right
        cv2.line(
            frame_copy, (x + w, y + h), (x + w - corner_len, y + h), (255, 255, 255), 3
        )
        cv2.line(
            frame_copy, (x + w, y + h), (x + w, y + h - corner_len), (255, 255, 255), 3
        )
...
def apply_heatmap_overlay(
    frame: np.ndarray, error_heatmap: np.ndarray, alpha: float = 0.3
) -> np.ndarray:
    """
...
def add_anomaly_indicator(
    ...
    cv2.rectangle(
        frame_copy, (bar_x, h - 22), (bar_x + bar_width, h - 12), bar_color, -1
    )
    cv2.putText(
        frame_copy,
        f"{confidence * 100:.0f}%",
...
def create_annotated_video(
    ...
    while True:
        ...
        with torch.no_grad():
            input_tensor = (
                torch.FloatTensor(normalized).unsqueeze(0).unsqueeze(0).to(device)
            )
...
def create_comparison_frame(
    ...
    heatmap_color = cv2.applyColorMap(
        cv2.resize(error_heatmap, (target_width, h)), cv2.COLORMAP_JET
    )
...
ANNOTATED_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "annotated_videos"
)
