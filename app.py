...
from storage.database import (
    init_database, save_video_analysis, get_all_videos,
    get_video_by_id, search_videos, delete_video, get_statistics
)
...
def run_analysis_task(task_id: str, file_path: str, output_path: str, crowd_threshold: int, confidence: float, original_filename: str):
    ...
    try:
        ...
        yolo = get_yolo_detector(crowd_threshold=crowd_threshold, confidence=confidence)
        stats = yolo.process_video(file_path, output_path, progress_callback=update_progress)
        ...
        video_id = save_video_analysis(
            ...
            anomaly_scores=[1.0 if r["is_anomaly"] else 0.0 for r in stats["frame_results"]],
...
async def analyze_yolo(
    ...
    output_filename = f"yolo_{task_id}.mp4" # Use MP4 extension
...
async def configure_email(
    ...
    success = config_email(smtp_server, smtp_port, sender_email, sender_password, admin_list)
